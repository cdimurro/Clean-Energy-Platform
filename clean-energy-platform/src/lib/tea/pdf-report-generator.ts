 ...
 * Controls investor-grade report generation for due diligence
 */
export interface InvestorReportConfig {
  template: 'investor' | 'technical' | 'executive' | 'government'
  ...
  // Rating
  overallRating: 'RECOMMENDED' | 'CONDITIONAL' | 'PROMISING' | 'NOT_RECOMMENDED'
  ratingConfidence: 'high' | 'medium' | 'low'

  // Claims validation results
  claimsValidation?: Array<{
    claim: string
    status: 'verified' | 'plausible' | 'unverified' | 'disputed'
...
export interface ComprehensiveTEAReportData {
  ...
  investorConfig?: InvestorReportConfig
}
...
export class EnhancedTEAReportGenerator {
  ...
  generateReport(): jsPDF {
    ...
    // 13a. Claims Validation (Investor Reports)
    if (this.data.investorConfig?.claimsValidation) {
      this.addClaimsValidation()
      this.addNewPage()
    }

    // 13b. Risk Assessment (Investor Reports)
    if (this.data.investorConfig?.risks) {
      this.addRiskAssessment()
      this.addNewPage()
    }

    // 13c. Investment Thesis (Investor Reports)
    if (this.data.investorConfig?.investmentThesis) {
      this.addInvestmentThesis()
      this.addNewPage()
    }

    // 13d. Due Diligence & Recommendations (Investor Reports)
    if (this.data.investorConfig?.dueDiligenceChecklist || this.data.investorConfig?.dataGaps) {
      this.addDueDiligenceRecommendations()
      this.addNewPage()
    }
  ...
  /**
   * Claims Validation Section - Shows validation status of company claims
   */
  private addClaimsValidation() {
    const claims = this.data.investorConfig?.claimsValidation
    if (!claims || claims.length === 0) return

    this.addSectionHeader('Claims Validation', 1)

    // Introduction
    this.addText(
      'This section presents the results of physics-based validation for key technology claims. ' +
      'Each claim is assessed against industry benchmarks, thermodynamic limits, and available evidence.'
    )
    ...
    const claimsData = claims.map(claim => {
      ...
      return [
        claim.claim,
        `${statusIcon} ${claim.status.toUpperCase()}`,
        riskColor,
        claim.physicsCheck ? 'Yes' : 'No',
      ]
    })

    this.addTable(
      'Claims Validation Summary',
      ['Claim', 'Status', 'Risk Level', 'Physics Check'],
      claimsData,
      { fontSize: 9 }
    )

    // Detailed claim analysis
    this.addSectionHeader('Detailed Claim Analysis', 2)

    claims.forEach((claim, index) => {
      this.checkPageBreak(40)

      // Claim header with colored status box
      const statusColors: Record<string, [number, number, number]> = {
        verified: COLORS.success,
        plausible: COLORS.warning,
        unverified: COLORS.textMuted,
        disputed: COLORS.error,
      }

      // Draw status indicator box
      this.doc.setFillColor(...(statusColors[claim.status] || COLORS.textMuted))
      this.doc.roundedRect(this.margin, this.currentY - 3, 4, 12, 1, 1, 'F')

      // Claim title
      this.doc.setFont(FONTS.body, 'bold')
      this.doc.setFontSize(11)
      this.doc.setTextColor(...COLORS.text)
      this.doc.text(`${index + 1}. "${claim.claim}"`, this.margin + 8, this.currentY + 4)
      this.currentY += 10
      ...
      this.doc.setFontSize(9)
      this.doc.setTextColor(...COLORS.textSecondary)
      const evidenceLines = this.doc.splitTextToSize(
  ...
  /**
   * Risk Assessment Section - Shows identified risks with likelihood/impact
   */
  private addRiskAssessment() {
    const risks = this.data.investorConfig?.risks
    if (!risks || risks.length === 0) return

    this.addSectionHeader('Risk Assessment', 1)

    this.addText(
      'This section presents a comprehensive risk assessment covering technical, market, regulatory, ' +
      'financial, and operational risks associated with the technology and investment.'
    )
    ...
    if (this.data.charts?.riskHeatmap) {
      this.checkPageBreak(100)
      this.addFigure('Risk Heat Map', this.data.charts.riskHeatmap, 150, 90)
    }

    // Detailed risks by category
    const categories = ['technical', 'market', 'regulatory', 'financial', 'operational'] as const
    categories.forEach(category => {
      const categoryRisks = risks.filter(r => r.category === category)
      if (categoryRisks.length === 0) return

      this.checkPageBreak(40)
      this.addSectionHeader(`${category.charAt(0).toUpperCase() + category.slice(1)} Risks`, 2)
      ...
      categoryRisks.forEach((risk, index) => {
        ...
        this.doc.setFontSize(10)
        this.doc.setTextColor(...COLORS.text)
        this.doc.text(`${index + 1}. ${risk.description}`, this.margin, this.currentY)
        this.currentY += 5
        ...
        const likelihoodColor = risk.likelihood === 'high' ? COLORS.error :
          risk.likelihood === 'medium' ? COLORS.warning : COLORS.success
        const impactColor = risk.impact === 'high' ? COLORS.error :
          risk.impact === 'medium' ? COLORS.warning : COLORS.success

        this.doc.setTextColor(...likelihoodColor)
        this.doc.text(`Likelihood: ${risk.likelihood.toUpperCase()}`, this.margin + 5, this.currentY)
        this.doc.setTextColor(...impactColor)
        this.doc.text(`Impact: ${risk.impact.toUpperCase()}`, this.margin + 50, this.currentY)
        this.currentY += 5
  ...
  /**
   * Investment Thesis Section - Summarizes investment rationale
   */
  private addInvestmentThesis() {
    const thesis = this.data.investorConfig?.investmentThesis
    if (!thesis) return

    this.addSectionHeader('Investment Thesis', 1)

    // Rating badge box at top
    const rating = this.data.investorConfig?.overallRating || 'CONDITIONAL'
    const confidence = this.data.investorConfig?.ratingConfidence || 'medium'

    const ratingColors: Record<string, [number, number, number]> = {
      RECOMMENDED: COLORS.success,
      CONDITIONAL: COLORS.warning,
      PROMISING: COLORS.primaryLight,
    ...
    this.doc.setFillColor(...(ratingColors[rating] || COLORS.textMuted))
    this.doc.roundedRect(boxX, this.currentY, boxWidth, boxHeight, 4, 4, 'F')

    // Rating text
    this.doc.setFont(FONTS.body, 'bold')
    this.doc.setFontSize(16)
    this.doc.setTextColor(...COLORS.white)
    this.doc.text(rating.replace('_', ' '), this.pageWidth / 2, this.currentY + 12, { align: 'center' })

    // Confidence text
    this.doc.setFontSize(9)
    this.doc.text(`Confidence: ${confidence.toUpperCase()}`, this.pageWidth / 2, this.currentY + 22, { align: 'center' })

    this.currentY += boxHeight + 15
    this.doc.setTextColor(0, 0, 0)

    // Value proposition
    this.addSectionHeader('Value Proposition', 2)
    this.addText(thesis.valuePropPosition)

    // Market opportunity
    this.addSectionHeader('Market Opportunity', 2)
    this.addText(thesis.marketOpportunity)

    // Competitive moat
    this.addSectionHeader('Competitive Moat', 2)
    this.addText(thesis.competitiveMoat)

    // Scaling pathway
    this.addSectionHeader('Scaling Pathway', 2)
    this.addText(thesis.scalingPathway)

    // Exit scenarios
    if (thesis.exitScenarios && thesis.exitScenarios.length > 0) {
      this.addSectionHeader('Exit Scenarios', 2)
      thesis.exitScenarios.forEach((scenario, i) => {
        this.addText(`${i + 1}. ${scenario}`)
      })
    }
  }

  /**
   * Due Diligence & Recommendations Section
   */
  private addDueDiligenceRecommendations() {
    this.addSectionHeader('Due Diligence & Recommendations', 1)

    // Data gaps section
    const dataGaps = this.data.investorConfig?.dataGaps
    if (dataGaps && dataGaps.length > 0) {
      this.addSectionHeader('Identified Data Gaps', 2)

      this.addText(
        'The following data gaps were identified during the assessment. ' +
        'Resolving these gaps would significantly improve confidence in the analysis.'
      )

      this.currentY += 5

      // Group by importance
      const critical = dataGaps.filter(g => g.importance === 'critical')
      const high = dataGaps.filter(g => g.importance === 'high')
      const medium = dataGaps.filter(g => g.importance === 'medium')
      const low = dataGaps.filter(g => g.importance === 'low')

      const renderGaps = (gaps: typeof dataGaps, label: string, color: [number, number, number]) => {
        ...
        this.doc.roundedRect(this.margin, this.currentY - 2, 3, 10, 1, 1, 'F')
        this.doc.setFont(FONTS.body, 'bold')
        this.doc.setFontSize(10)
        this.doc.setTextColor(...COLORS.text)
        this.doc.text(`${label} (${gaps.length})`, this.margin + 6, this.currentY + 4)
        this.currentY += 10
        ...
        gaps.forEach((gap) => {
          ...
          this.doc.setFontSize(9)
          this.doc.setTextColor(...COLORS.text)
          this.doc.text(`- ${gap.gap}`, this.margin + 8, this.currentY)
          this.currentY += 4

          this.doc.setTextColor(...COLORS.textMuted)
          this.doc.setFontSize(8)
          this.doc.text(`  Request: ${gap.dataRequest}`, this.margin + 8, this.currentY)
          this.currentY += 5
        })
      ...
      renderGaps(critical, 'Critical Priority', COLORS.error)
      renderGaps(high, 'High Priority', COLORS.warning)
      renderGaps(medium, 'Medium Priority', COLORS.primaryLight)
      renderGaps(low, 'Low Priority', COLORS.textMuted)
    }

    // Due diligence checklist
    const checklist = this.data.investorConfig?.dueDiligenceChecklist
    if (checklist && checklist.length > 0) {
      this.checkPageBreak(50)
      this.addSectionHeader('Due Diligence Checklist', 2)
      ...
      this.doc.setFontSize(10)
      this.doc.setTextColor(...COLORS.text)
      this.doc.text(`Due Diligence Progress: ${complete}/${total} items complete (${percentage}%)`, this.margin, this.currentY)
      this.currentY += 10
    }

    // Next steps
    this.checkPageBreak(40)
    this.addSectionHeader('Recommended Next Steps', 2)

    const nextSteps = [
      'Request detailed mass and energy balance data from the company',
      'Conduct site visit to operational pilot/demo facility',
      'Engage independent technical advisor for process validation',
      'Review third-party emissions testing and environmental compliance',
      'Analyze customer contracts and offtake agreements',
      'Review intellectual property portfolio and freedom-to-operate',
    ]
    ...
    nextSteps.forEach((step, i) => {
      ...
      this.doc.setFontSize(9)
      this.doc.setTextColor(...COLORS.text)
      this.doc.text(`${i + 1}. ${step}`, this.margin + 5, this.currentY)
      this.currentY += 5
    })
