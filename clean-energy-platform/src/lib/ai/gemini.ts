...
// Lazy-initialized Gemini client (reads API key when first used, not at module load)
...
// Always use Gemini 3 Flash Preview for all operations
// ============================================================================
const GEMINI_MODEL = 'gemini-3-flash-preview' as const

const MODELS: Record<GeminiModel, string> = {
  'flash-lite': GEMINI_MODEL,
  flash: GEMINI_MODEL,
  pro: GEMINI_MODEL,
}
...
export async function generateText(
  ...
  try {
    ...
    const generativeModel: GenerativeModel = getGenAI().getGenerativeModel({
      model: MODELS[model],
      generationConfig,
    })
...
export async function generateWithTools(
  ...
  try {
    ...
    const generativeModel: GenerativeModel = getGenAI().getGenerativeModel(modelConfig)
