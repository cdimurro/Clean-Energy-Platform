...
// ============================================================================
// ASSESSMENT PLAN TYPES (Editable Plan Feature)
// ============================================================================

/**
 * Source attribution for plan assumptions
 * Tracks where each value came from for transparency
 */
export type AssumptionSourceType = 'extracted' | 'default' | 'calculated' | 'user'

export interface AssumptionSource {
  type: AssumptionSourceType
  documentName?: string // For 'extracted' - name of uploaded document
  pageNumber?: number // For 'extracted' - page where value was found
  technologyType?: string // For 'default' - which technology defaults were used
  calculation?: string // For 'calculated' - formula description
  originalValue?: string | number // Value before user modification
  modifiedAt?: Date // When user last modified
}

/**
 * Categories for grouping assumptions in the plan editor
 */
...
/**
 * Single assumption with full metadata and source attribution
 */
export interface PlanAssumption {
  id: string
  category: AssumptionCategory
  key: string // Internal key (e.g., 'discount_rate')
  label: string // Display label (e.g., 'Discount Rate (WACC)')
  value: string | number
  unit: string // e.g., '%', '$/kW', 'years'
  source: AssumptionSource
  description?: string // Help text explaining the assumption
  validRange?: { min: number; max: number } // For numeric validation
  isRequired: boolean // Whether this assumption is required
  isEditable: boolean // Whether user can modify
}

/**
 * Methodology analysis configuration
 * Controls which analyses are included in the assessment
 */
export interface MethodologyConfig {
  id: string
  name: string // e.g., 'Technology Deep Dive'
  description: string // What this analysis covers
  enabled: boolean // Whether to include in execution
  approach: string // AI-generated methodology description
  requiredData?: string[] // What data is needed for this analysis
  estimatedDuration: string // e.g., '2-3 minutes'
}

/**
 * Plan section configuration (for report generation)
 */
...
/**
 * Missing data item with impact assessment
 */
...
export interface PlanModification {
  fieldPath: string // e.g., 'assumptions.financial.discount_rate'
...
/**
 * Enhanced Assessment Plan
 * Full editable plan with methodology, assumptions, and source attribution
 */
export interface EnhancedAssessmentPlan {
  id: string
  version: number
  ...
  methodology: {
    overview: string // AI summary of the assessment approach
    analyses: MethodologyConfig[] // Configurable analyses
    limitations: string[] // Known limitations of the analysis
  }

  // Grouped assumptions with source attribution
  assumptions: {
    financial: PlanAssumption[]
    technical: PlanAssumption[]
    capital_costs: PlanAssumption[]
    operating_costs: PlanAssumption[]
    revenue: PlanAssumption[]
    environmental: PlanAssumption[]
  }

  // Claims to validate (from uploaded documents)
  identifiedClaims: Array<{
    id: string
    claim: string
    source: string
